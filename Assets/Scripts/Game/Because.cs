...
public class Because : MonoBehaviour
{
    ...
    public bool CanMoveToDir(Vector2 dir)
    {
        ...
        if (!hit || hit.collider.tag == "Door")
        {
            ...
            result.GetComponent<Result>().CanMoveToDir(dir);
            return true;
        }
        // // Debug
        // else
    ...
    void DebugDarwLine(Vector2 dir)
    {
        if (dir == Vector2.up || dir == Vector2.down)
        {
            Debug.DrawLine(_self.position + new Vector3(0.87f * edge, 0, 0) + (Vector3)dir * edge, _self.position + new Vector3(0.97f * edge, 0, 0) + (Vector3)dir * edge * 2f, new Color(255, 0, 0, 255));
            Debug.DrawLine(_self.position + new Vector3(-0.87f * edge, 0, 0) + (Vector3)dir * edge, _self.position + new Vector3(-0.97f * edge, 0, 0) + (Vector3)dir * edge * 2f, new Color(255, 0, 0, 255));
        }
        else
        {
            Debug.DrawLine(_self.position + new Vector3(0, 0.87f * edge, 0) + (Vector3)dir * edge, _self.position + new Vector3(0, 0.97f * edge, 0) + (Vector3)dir * 2f, new Color(255, 0, 0, 255));
            Debug.DrawLine(_self.position + new Vector3(0, -0.87f * edge, 0) + (Vector3)dir * edge, _self.position + new Vector3(0, -0.97f * edge, 0) + (Vector3)dir * 2f, new Color(255, 0, 0, 255));
        }
    }
}
